...
execute if entity @s[gamemode=!creative,gamemode=!spectator] unless entity @s[nbt={RootVehicle:{Entity:{id:"minecraft:boat"}}}] if block ~ ~-.05 ~ water if block ~1 ~-.05 ~ water if block ~-1 ~-.05 ~ water if block ~ ~-.05 ~1 water if block ~ ~-.05 ~-1 water if block ~1 ~-.05 ~1 water if block ~-1 ~-.05 ~-1 water if block ~-1 ~-.05 ~1 water if block ~1 ~-.05 ~-1 water if block ~ ~0.05 ~ air run tag @s add coah.jesus
